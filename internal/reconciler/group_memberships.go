...
func (r *Reconciler) GroupMembership(ctx context.Context, gid, oktaGID string) error {
	...
	for _, uid := range group.Members {
		...
		if !r.dryrun {
			...
			logger.Info("dryrun adding user to okta group", zap.String("user.email", user.Email))
		}
	}
	...
	for _, oktaUID := range oktaGroupMembers {
		...
		if !r.dryrun {
			...
			logger.Info("dryrun removing user from okta group", zap.String("okta.user.id", oktaGID))
		}
	}
...
func (r *Reconciler) GroupMembershipDelete(ctx context.Context, gid, uid string) (string, string, error) {
	...
	if !r.dryrun {
		...
		logger.Info("dryrun removing user from okta group", zap.String("user.email", user.Email))
	}
